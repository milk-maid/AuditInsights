...
contract CalldataTrial {

    function transfer(uint256 amount, address to) external {}

    function getFunctionSignatureHash() public pure returns (bytes32) {
        return keccak256("transfer(uint256,address)");
    }

    function getSelector() public pure returns (bytes4) {
        bytes32 functionSignatureHash = getFunctionSignatureHash();
        return bytes4(functionSignatureHash);
    }

    function getFunctSigHash() public pure returns (bytes32) {
        return keccak256(bytes("transfer(uint256,address)"));
    }

    function getFuncSelector() public pure returns (bytes4) {
        bytes32 funcSig = getFunctSigHash();
        return bytes4(funcSig);
    }


    function getSelect(string memory functionSignature) public pure returns (bytes4) {
        return bytes4(keccak256(bytes(functionSignature)));
    }

    function getCalldata(uint256 _amount, address _to) public pure returns (bytes memory) {
        return abi.encode(_amount,_to);
    }

    function decodeCalldata() public pure returns (uint256, address) {
        bytes memory callData = getCalldata(1300655506, 0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45);
        return abi.decode(callData, (uint256, address));    
    }

    function callFunction(uint256 _amount, address _to) public pure returns (bytes memory) {
        return abi.encodeWithSignature("transfer(uint256,address)", _amount, _to);
    }
}
